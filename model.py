...
class Generator_Model(tf.keras.Model):
    def __init__(self):
        ...
        model.add(Dense(4*4*512, input_shape=(args.z_dim,)))
    ...
    def call(self, input):
        ...
        :param input: a batch of random noise vectors, shape=[batch_size, z_dim]
    ...
    def loss_function(self, disc_fake_output):
        ...
        """
        return self.loss_f(tf.ones_like(disc_fake_output), disc_fake_output)
...
class Discriminator_Model(tf.keras.Model):
    ...
    def call(self, input):
        """
        Executes the discrimator model on a batch of input images and outputs whether it is real or fake
        :param input: a batch of images, shape=[batch_size, height, width, channels]
    ...
    def loss_function(self, disc_real_output, disc_fake_output):
        ...
        """
        real_loss = self.loss_f(tf.ones_like(disc_real_output), disc_real_output)
        fake_loss = self.loss_f(tf.zeros_like(disc_fake_output), disc_fake_output)
...
def train(generator, discriminator, dataset_iterator, manager):
    ...
    :param dataset_ierator:
    :return: average FID over the epoch
    """
    all_fids = []
    # Loop over our data until we run out
    for iteration, batch in enumerate(dataset_iterator):
        # Sample randomness to feed generator
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            # Feed the discriminator with the generated and real images
            disc_fake_output = discriminator.call(gen_output)
            disc_real_output = discriminator.call(batch)
        ...
        if iteration % args.log_every == 0:
            print('Iteration %d: Gen loss = %g | Discrim loss = %g' % (iteration, gen_loss, disc_loss))

        # Get generator and discriminator gradients
        gen_gradients = gen_tape.gradient(gen_loss, generator.trainable_variables)
        disc_gradients = disc_tape.gradient(disc_loss, discriminator.trainable_variables)

        # Update generator and discriminator weights with gradients
        generator.optimizer.apply_gradients(zip(gen_gradients, generator.trainable_variables))
        # Only update discriminator on even batches - so you update the generator more often
        if iteration % 2 == 0:
        ...
        # Calculate inception distance
        if iteration % 10 == 0:
            fid_ = fid_function(batch, gen_output)
            all_fids.append(fid_)
            print('**** INCEPTION DISTANCE: %g ****' % fid_)
    return sum(all_fids) / len(all_fids)
